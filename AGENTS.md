...
| Action | Command |
|--------|---------|
| Build debug APK | `cd android && ./gradlew assembleDebug` (requires Gradle **9.4.1**, AGP **9.2.1**, SDK **36**) |
| Build release AAB | `cd android && ./gradlew bundleRelease` → `app/build/outputs/bundle/release/app-release.aab` |
| APK output | `android/app/build/outputs/apk/debug/app-debug.apk` |

The repo includes **`gradlew`** and **`gradle-wrapper.properties`** (Gradle 9.4.1). `./gradlew` is the preferred build entry point.
